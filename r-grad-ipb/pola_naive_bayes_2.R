...
extract_feature <- function(dir_path, width, height, label, add_label = TRUE) {
  ...
  ## ambil semua citra dalam direktori
...
# panggil fungsi ekstraksi fitur citra 
{
  dir_path <- (choose.dir())
  buah_pir <- extract_feature(dir_path,60,45,1,TRUE)
  dir_path <- (choose.dir())
  buah_naga <- extract_feature(dir_path,60,45,2,TRUE)
  dir_path <- (choose.dir())
  buah_jambu <- extract_feature(dir_path,60,45,3,TRUE)
}
